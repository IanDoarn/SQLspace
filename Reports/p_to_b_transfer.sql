...
SELECT
  ...
  to_char(completed_date::timestamp at time zone '""Etc/GMT+3"', 'DD/MM/YYYY HH12:MI:SS PM') as completed_date,
	CASE 
		...
		when t.from_container_type = 4 then 'From Loan'
	END AS from_container_type,
	CASE
		WHEN t.to_container_type = 1 THEN 'Into Bin'
		WHEN t.to_container_type = 2 THEN 'Into Kit'
		when t.to_container_type = 3 then 'Into Package'
		when t.to_container_type = 4 then 'Into Loan'
...
WHERE
	...
	and t.status in (1,2)
	and t.completed_date between '2016-06-01' and '2016-06-30'
	and t.completed_date != '2016-05-31'
	and t.transfer_type = 6
),
s2 as (
SELECT
  p.product_number,
